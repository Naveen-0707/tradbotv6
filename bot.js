...
function calcQty(risk, entry) {
  ...
  return Math.min(Math.max(1, Math.min(maxByRisk, maxByCapital, maxByPct)), MAX_QTY);
}

// ─── NSE HOLIDAY CALENDAR ────────────────────────────────────────────────────
// Fetches trading holidays from Upstox /v2/market/holidays/NSE.
...
async function fetchNSEHolidays() {
  try {
    const r = await fetchR(API_HOST, "/v2/market/holidays/NSE", "GET", authH());
    if (r.status === 200 && Array.isArray(r.data?.data)) {
      const MON = { Jan:"01",Feb:"02",Mar:"03",Apr:"04",May:"05",Jun:"06",
                    Jul:"07",Aug:"08",Sep:"09",Oct:"10",Nov:"11",Dec:"12" };
      const holidays = r.data.data
        .filter(h => (h.holiday_type || "").toLowerCase().includes("trading"))
        .map(h => {
          // Upstox returns "26-Jan-2026" → normalise to "2026-01-26"
          const p = (h.date || "").split("-");
          if (p.length !== 3) return null;
          const iso = `${p[2]}-${MON[p[1]] || "00"}-${p[0].padStart(2, "0")}`;
  ...
  try {
    return JSON.parse(fs.readFileSync(HOL_FILE, "utf8")).holidays || [];
  } catch { return []; }
}

// Synchronous — reads cached file only (called from isMarket() hot path)
function isNSEHoliday() {
  try {
    const cached = JSON.parse(fs.readFileSync(HOL_FILE, "utf8"));
    const today  = todayStr();
    const hit    = (cached.holidays || []).find(h => h.date === today);
    if (hit) {
      // Only log once per day to avoid log spam
      if (!isNSEHoliday._loggedDate || isNSEHoliday._loggedDate !== today) {
        isNSEHoliday._loggedDate = today;
        log(`📅 NSE Holiday: ${hit.desc} — market closed today`, "INFO");
        notify("📅 NSE Holiday", hit.desc);
      }
      return true;
    }
  } catch { /* no cache yet — assume open */ }
  return false;
}
isNSEHoliday._loggedDate = "";
