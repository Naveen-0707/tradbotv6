...
const HOL_FILE = path.join(DIR, "nse_holidays.json"); // cached NSE trading holidays
...
const MLOSS        = () => getModeValue("dailyLossLimit", 500, v => Number(v) || 500);
...
const normStockName = name => String(name || "").replace(/[^A-Z0-9]/gi, "").toUpperCase();
const liveShortAllowlist = () => (
  Array.isArray(CFG.liveShortAllowlist)
    ? CFG.liveShortAllowlist.map(normStockName).filter(Boolean)
    : []
);
...
function writeBotStatus(extra = {}) {
  const payload = {
    ...
    currentLogFile: path.basename(currentLogFile),
    ...extra,
  };
  try { fs.writeFileSync(BOT_STATUS_FILE, JSON.stringify(payload, null, 2)); } catch {}
}

function clearBotStatus(reason = "stopped") {
  try {
    fs.writeFileSync(BOT_STATUS_FILE, JSON.stringify({
      alive: false,
      botStopped: true,
      pid: process.pid,
      reason,
...
const isMarket = () => {
  ...
  if (m < 555 || m > 930)  return false;       // outside session
  if (isNSEHoliday())      return false;       // NSE trading holiday
  return true;
};
...
async function fetchLTP(keys) {
  ...
  try {
    const r = await fetchR(API_HOST, `/v2/market-quote/quotes?instrument_key=${param}`, "GET", authH());
...
function dailyModePnL() {
  const today = todayStr();
  return trades
    .filter(t => t.date === today && (!!t.paper) === PAPER())
    .reduce((s, t) => s + (t.pnl !== null ? t.pnl : (t.livePnl || 0)), 0);
}

function calcTradePnLAtExit(trade, exitPrice, charges = COSTS() * 2) {
  const gross = trade.direction === "BUY"
...
function calcQty(risk, entry) {
  ...
  const maxByPct     = Math.floor((avail * 0.30) / entry); // BUG-4 FIX: 30% single-trade cap
  const MAX_QTY      = 500; // safety cap — prevents runaway qty on tiny risk values
  const cappedQty = Math.min(maxByRisk, maxByCapital, maxByPct);
  if (cappedQty <= 0) return 0;
  return Math.min(cappedQty, MAX_QTY);
}

// ─── NSE HOLIDAY CALENDAR ────────────────────────────────────────────────────
// Fetches trading holidays from Upstox /v2/market/holidays and filters NSE closures.
// Cached locally in nse_holidays.json, refreshed weekly on Sunday at 3 AM.
// isNSEHoliday() is called synchronously from isMarket() — reads from cache only.

let nseHolidays = [];
let loggedHolidayDate = "";

function normaliseHolidayDate(raw) {
  ...
  return `${p[2]}-${MON[p[1]] || "00"}-${p[0].padStart(2, "0")}`;
}

function setHolidayCache(holidays) {
  nseHolidays = Array.isArray(holidays) ? holidays : [];
  return nseHolidays;
}

function loadHolidayCacheFromDisk() {
  try {
    const cached = JSON.parse(fs.readFileSync(HOL_FILE, "utf8"));
    return setHolidayCache(cached.holidays || []);
  } catch {
    return setHolidayCache([]);
  }
}

async function fetchNSEHolidays() {
  try {
    const r = await fetchR(API_HOST, "/v2/market/holidays", "GET", authH());
    if (r.status === 200 && Array.isArray(r.data?.data)) {
      const holidays = r.data.data
        .filter(h => {
          const closed = Array.isArray(h.closed_exchanges) ? h.closed_exchanges : [];
          return (h.holiday_type || "").toLowerCase().includes("trading") &&
            (closed.length === 0 || closed.includes("NSE"));
        })
        .map(h => {
          const iso = normaliseHolidayDate(h.date || "");
          if (!iso) return null;
          return { date: iso, desc: h.description || "" };
        })
        .filter(Boolean);
      const payload = { fetched: new Date().toISOString(), holidays };
      fs.writeFileSync(HOL_FILE, JSON.stringify(payload, null, 2));
      setHolidayCache(holidays);
      log(`📅 NSE holidays refreshed: ${holidays.length} trading holidays cached`, "INFO");
      return holidays;
    }
    log(`⚠️ NSE holiday API returned status ${r.status} — keeping cache`, "WARN");
  } catch (e) {
    log(`⚠️ NSE holiday fetch error: ${e.message} — keeping cache`, "WARN");
  }
  // Fallback: return whatever is cached
  return nseHolidays.length > 0 ? nseHolidays : loadHolidayCacheFromDisk();
}

// Reads in-memory cache only (called from isMarket() hot path)
function isNSEHoliday() {
  const today = todayStr();
  const hit   = nseHolidays.find(h => h.date === today);
  if (hit) {
    if (loggedHolidayDate !== today) {
      loggedHolidayDate = today;
      log(`📅 NSE Holiday: ${hit.desc} — market closed today`, "INFO");
      notify("📅 NSE Holiday", hit.desc);
    }
    return true;
  }
  return false;
...
// ─── BLOCKED EVENT GATE ───────────────────────────────────────────────────────
// config.json: "blockedEvents": [{ "date":"2026-06-06","from":555,"to":930,"reason":"RBI Policy" }]
// from/to are minOfDay values (555 = 9:15 AM, 930 = 3:30 PM).
// Omit from/to to block the entire session.

function isBlockedEvent() {
  const events = CFG.blockedEvents || [];
  if (events.length === 0) return false;
  const today = todayStr();
  const m     = minOfDay();
  const ev    = events.find(e =>
    e.date === today &&
    m >= (e.from !== undefined ? e.from : 555) &&
    m <  (e.to   !== undefined ? e.to   : 930)
  );
  if (ev) {
    log(`📅 Blocked event: "${ev.reason}" — scan skipped`, "WARN");
    return true;
  }
  return false;
}

function canLiveShortStock(stock) {
  if (!stock) return false;
  return liveShortAllowlist().includes(normStockName(stock.name));
}

// ─── NIFTY STATE — extended with spike guard + VIX ───────────────────────────
const niftyState = {
  ltp: 0, prevClose: 0, direction: 0, lastUpdated: 0,
  // Spike guard
  prevLtp: 0, prevLtpTs: 0,
  macroAlert: false, macroAlertTs: 0,
  // VIX gate
  vix: 0, vixPenalty: 0,
};
...
async function refreshNiftyState() {
  ...
  try {
    ...
    // ── SPIKE GUARD ──────────────────────────────────────────────────────
    // Triggers macro alert if Nifty moves >0.4% within a 3-minute window.
    // Yesterday's ICICIBANK/MARUTI scenario: Nifty reversed at ~13:40,
    // all 4 SELL SLs were never hit because OCO bug; with this guard the
    // spike itself would have triggered emergency exits within one 5s cycle.
    if (niftyState.prevLtp > 0 && niftyState.prevLtpTs > 0) {
      const elapsedSec = (Date.now() - niftyState.prevLtpTs) / 1000;
      const pctMove    = Math.abs((ltp - niftyState.prevLtp) / niftyState.prevLtp) * 100;
      if (elapsedSec < 180 && pctMove > 0.4) {
        if (!niftyState.macroAlert) {
          niftyState.macroAlert   = true;
          niftyState.macroAlertTs = Date.now();
          const dir = ltp > niftyState.prevLtp ? "▲" : "▼";
          log(`🚨 MACRO ALERT: Nifty ${dir}${pctMove.toFixed(2)}% in ${elapsedSec.toFixed(0)}s — scans blocked, emergency exits armed`, "WARN");
          notify("🚨 Macro Alert", `Nifty ${dir}${pctMove.toFixed(2)}% spike — trading paused`);
        }
      }
    }
    // Auto-clear after 15 min if market stabilises
    if (niftyState.macroAlert) {
      const alertAgeMins = (Date.now() - niftyState.macroAlertTs) / 60000;
      if (alertAgeMins > 15) {
        niftyState.macroAlert = false;
        log("✅ Macro alert cleared — market stable for 15 min, trading resumed", "INFO");
        notify("✅ Macro Cleared", "Trading resumed");
      }
    }
    // Rolling LTP reference for next spike check
    niftyState.prevLtp   = ltp;
    niftyState.prevLtpTs = Date.now();
    ...
    // ── VIX GATE ─────────────────────────────────────────────────────────
    // India VIX > 20 = elevated fear (+1 to score req), > 25 = extreme (+2).
    // Non-fatal: if VIX fetch fails, vixPenalty stays at last known value.
    try {
      const vix = await fetchIndexLTP("NSE_INDEX|India VIX");
      if (vix && vix > 0) {
        const prevPenalty     = niftyState.vixPenalty;
        niftyState.vix        = vix;
        niftyState.vixPenalty = vix > 25 ? 2 : vix > 20 ? 1 : 0;
        if (niftyState.vixPenalty !== prevPenalty) {
          log(`📊 VIX ${vix.toFixed(1)} → score penalty ${niftyState.vixPenalty > 0 ? `+${niftyState.vixPenalty}` : "none"}`, "INFO");
        }
      }
    } catch { /* non-fatal — keep previous vixPenalty */ }
...
async function simulatePaperOCO() {
  ...
  // ROOT CAUSE FIX: Upstox v3 /ltp response keys may use ":" separator while
  // instrument keys use "|" (e.g. "NSE_EQ|INE302A01020" vs "NSE_EQ:INE302A01020").
  // Normalize both sides before lookup so ltpData[key] never returns undefined.
  const normKey = k => k.replace(/[|:]/g, "_").toUpperCase();
  const ltpByNorm = {};
  for (const [k, v] of Object.entries(ltpData)) ltpByNorm[normKey(k)] = v;

  let changed = false;

  for (const [key, trade] of Object.entries(keyMap)) {
    const ltpEntry = ltpByNorm[normKey(key)]; // separator-agnostic lookup
    ...
    // ── MACRO EMERGENCY EXIT ──────────────────────────────────────────────
    // If a spike event is active AND the trade is losing, exit immediately
    // at current LTP rather than waiting for SL price to be reached.
    // This is what would have saved the MARUTI/ICICIBANK trades at 13:40.
    if (niftyState.macroAlert && trade.livePnl < 0) {
      const charges = COSTS() * 2;
      const pnl     = +(trade.livePnl - charges);
      trade.status  = "MACRO_EXIT";
      trade.pnl     = pnl;
      trade.ltp     = +ltp.toFixed(2);
      markLoss(trade.name);
      log(`🚨 MACRO EXIT: ${trade.name} [${trade.strategy}] @ ₹${ltp} P&L:₹${pnl.toFixed(0)} (spike protection, incl ₹${charges} charges)`, "WARN");
      notify(`🚨 Macro Exit — ${trade.name}`, `₹${pnl.toFixed(0)} (spike protection)`);
      continue;
    }
    ...
    if (slHit) {
      const charges = COSTS() * 2;
      const pnl = calcTradePnLAtExit(trade, trade.sl, charges);
...
// ─── LIVE OCO CHECK ───────────────────────────────────────────────────────────
let liveOcoRunning = false;
async function checkLiveOCO() {
  if (liveOcoRunning) return;
  liveOcoRunning = true;
  ...
  try {
    for (const trade of open) {
      try {
      // ── MACRO EMERGENCY EXIT (LIVE) ───────────────────────────────────────
      // If a spike is active, place a MARKET order to exit immediately rather
      // than waiting for the SL order to be triggered by the exchange.
      if (niftyState.macroAlert) {
        const allStockList = [...STOCKS.tier1, ...STOCKS.tier2, ...STOCKS.tier3];
        ...
        if (stock) {
          try {
            ...
            const normK    = k => k.replace(/[|:]/g, "_").toUpperCase();
            const ltpEntry = Object.entries(ltpData).find(([k]) => normK(k) === normK(stock.key));
            ...
            if (freshLtp) {
              ...
              if (unrealised < 0) {
                log(`🚨 MACRO EXIT LIVE: ${trade.name} — placing market exit order`, "WARN");
                const exitTx = trade.direction === "BUY" ? "SELL" : "BUY";
                await placeOrd({
                  quantity: trade.qty, product: "I", validity: "DAY", price: 0,
                  tag: "FCB_MACRO_EXIT", instrument_token: stock.key,
                  order_type: "MARKET", transaction_type: exitTx,
                  disclosed_quantity: 0, trigger_price: 0, is_amo: false, slice: false,
                });
                const charges = COSTS() * 2;
                const pnl     = +(unrealised - charges);
                trade.status  = "MACRO_EXIT";
                trade.pnl     = pnl;
                markLoss(trade.name);
                saveTrades();
                log(`🚨 MACRO EXIT LIVE: ${trade.name} P&L ₹${pnl.toFixed(0)} (incl ₹${charges} charges)`, "WARN");
                notify(`🚨 Macro Exit — ${trade.name}`, `₹${pnl.toFixed(0)} (spike protection)`);
            ...
            log(`⚠️ Macro exit LTP fetch failed ${trade.name}: ${e.message}`, "WARN");
          }
        }
      }
...
async function execTrade(signal) {
  ...
  try {
    ...
    if (stock) {
      ...
      // BUG-2 FIX: v3 API returns depth.buy[]/depth.sell[], not depth.top[] (v2)
      if (quote?.depth?.buy?.[0] && quote?.depth?.sell?.[0]) {
  ...
  if (!PAPER() && signal.direction === "SELL" && !canLiveShortStock(stock)) {
    log(`🚫 LIVE short skipped for ${signal.name} — add it to config.liveShortAllowlist only after verifying broker/NSE eligibility`, "WARN");
    return;
  }
...
async function scan(strategyNames) {
  ...
  scanning = true;
  try {
    // BUG #10: refresh Nifty before every scan (also updates spike guard + VIX)
    await refreshNiftyState();

    // ── GATE: macro spike alert ───────────────────────────────────────────
    if (niftyState.macroAlert) {
      log("🚨 Macro alert active — scan blocked until market stabilises (15 min)", "WARN");
      return;
    }

    // ── GATE: known scheduled event (RBI/Budget/etc from config.json) ─────
    if (isBlockedEvent()) return;

    // ── EFFECTIVE SCORE THRESHOLD = base + VIX penalty ────────────────────
    // VIX > 20 → +1, VIX > 25 → +2. Raises bar on high-fear days.
    const effectiveThreshold = SCORE_THRESH() + (niftyState.vixPenalty || 0);
    ...
    const vixStr    = niftyState.vix > 0 ? ` VIX:${niftyState.vix.toFixed(1)}${niftyState.vixPenalty > 0 ? `(+${niftyState.vixPenalty})` : ""}` : "";
    log(
      `🚀 Scan: [${strategyNames.join("+")}] ${stocks.length} stocks` +
      ` | Nifty:${niftyIcon}${niftyState.ltp > 0 ? niftyState.ltp.toFixed(0) : "?"}${vixStr}` +
      ` | Score≥${effectiveThreshold}${niftyState.vixPenalty > 0 ? ` (base:${SCORE_THRESH()}+vix:${niftyState.vixPenalty})` : ""}`,
      "INFO"
    );
    ...
    for (let i = 0; i < stocks.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.allSettled(
        ...
        try {
          ...
          if (rawSig && rawSig.score < effectiveThreshold) {
            nearMisses.push({ name: stock.name, score: rawSig.score, strategy: rawSig.strategy, direction: rawSig.direction });
          }
          const signal = rawSig && rawSig.score >= effectiveThreshold ? rawSig : null;
  ...
  const signals = allResults
    ...
    if (signals.length === 0) {
      if (nearMisses.length > 0) {
        // BUG-10 FIX: was hardcoded s1–s5; score 6 silently dropped when threshold ≥ 7
        const buckets = {};
        ...
        const bucketStr = Object.entries(buckets)
          ...
          .map(([k, v]) => `${k.slice(1)}:${v}`)
          .join(" | ");
        log(`📊 Near-miss dist → ${bucketStr} (threshold:${effectiveThreshold}${niftyState.vixPenalty > 0 ? ` base:${SCORE_THRESH()}+vix:${niftyState.vixPenalty}` : ""})`, "SCAN");
...
let squaredOffToday = false; // BUG-3 FIX: prevents double-fire if loop skips exactly minute 920

async function loop() {
  if (botStopped) return;

  // Midnight reset of squaredOffToday flag
  if (minOfDay() < 555 && squaredOffToday) squaredOffToday = false;

  // BUG-3 FIX: was === 920 (could be skipped if loop tick takes >30s).
  // Now: >= 920 + one-shot flag so it fires exactly once per day.
  if (!squaredOffToday && minOfDay() >= 920) {
  ...
  if (m !== lastStatusMin && m % 5 === 0) {
    lastStatusMin = m;
    const pnl = dailyModePnL();
    const macroTag = niftyState.macroAlert ? " 🚨MACRO" : "";
    const vixTag   = niftyState.vix > 0 ? ` VIX:${niftyState.vix.toFixed(1)}` : "";
    log(
      `⏰ ${istTime()} | ${sched.label}` +
      ` | P&L:₹${pnl.toFixed(0)}` +
      ` | Live:${tradesToday()}/${MTRD()} Paper:${paperTradesToday()}` +
      ` | ${PAPER() ? "PAPER" : "LIVE"}` +
      ` | Score≥${SCORE_THRESH() + (niftyState.vixPenalty || 0)}` +
      ` | Avail:₹${availableBalance().toFixed(0)}` +
      vixTag + macroTag,
      "STATUS"
    );
    if (m >= 525 && m <= 960) notifyPersistent(`${sched.label} | ₹${pnl.toFixed(0)} | ${PAPER() ? "PAPER" : "LIVE"}${macroTag}`);
  }
...
function watchCMD() {
  ...
  const processCMD = async () => {
    ...
    processing = true;
    try {
      const raw = await fs.promises.readFile(CMD_FILE, "utf8");
      ...
      switch (cmd.cmd) {
        ...
        case "stop":
          botStopped = true;
          writeBotStatus({ reason: "stopped_by_ui" });
        ...
        case "resume":
          botStopped = false;
          writeBotStatus({ reason: "resumed_by_ui" });
          log("Bot resumed via UI", "INFO");
          notify("FCB Bot V6", "Resumed from UI");
          break;
        ...
        case "manual_paper":
          if (cmd.trade) {
            ...
            if (!hasOpenPosition(t.name, trades)) {
              ...
              saveTrades();
              log(`UI Paper synced: ${t.direction} ${t.qty}x ${t.name}`, "TRADE");
            }
          }
          break; 
        ...
        default:
          log(`Unknown command: ${cmd.cmd}`, "WARN");
      }
    } catch {
      // cmd file may not exist yet or may be mid-write
    } finally {
      processing = false;
    }
  };
  fs.watchFile(CMD_FILE, { interval: 1000 }, () => { void processCMD(); });
  void processCMD();
}
...
async function start() {
  ...
  // ── NSE HOLIDAY CALENDAR INIT ─────────────────────────────────────────────
  // Fetch from Upstox on first run or if cache is older than 7 days.
  // isMarket() reads this synchronously on every loop tick.
  try {
    let needRefresh = true;
    loadHolidayCacheFromDisk();
    if (fs.existsSync(HOL_FILE)) {
      const cached = JSON.parse(fs.readFileSync(HOL_FILE, "utf8"));
      const ageMs  = Date.now() - new Date(cached.fetched || 0).getTime();
      if (ageMs < 7 * 86400000) {
        needRefresh = false;
        log(`📅 NSE holidays: cached (${(cached.holidays || []).length} days, age ${Math.floor(ageMs/86400000)}d)`, "INFO");
      }
    }
    if (needRefresh) await fetchNSEHolidays();
  } catch (e) { log(`⚠️ Holiday init: ${e.message}`, "WARN"); }

  // Weekly auto-refresh: every Sunday at 3 AM IST
  setInterval(async () => {
    const d = istNow();
    if (d.getDay() === 0 && d.getHours() === 3) {
      log("📅 Weekly NSE holiday refresh...", "INFO");
      await fetchNSEHolidays();
    }
    ...
    for (const trade of openLive) {
      try {
        if (trade.slOrderId) {
          const s = await getOrderStatus(trade.slOrderId);
          if (s?.status === "complete") {
            const pnl = calcTradePnLAtExit(trade, trade.sl);
            trade.status = "STOPPED_OUT";
            trade.pnl    = pnl;
            markLoss(trade.name);
  ...
  watchCMD();
  await loop();
  writeBotStatus({ reason: "loop_started" });
  ...
  setInterval(() => writeBotStatus(), 15000);
...
// ─── GRACEFUL SHUTDOWN ────────────────────────────────────────────────────────
process.on("SIGINT",  () => { clearBotStatus("sigint"); log("Stopped (SIGINT)",  "INFO"); exec("termux-notification-remove 9999", () => {}); exec("termux-wake-unlock", () => {}); process.exit(0); });
process.on("SIGTERM", () => { clearBotStatus("sigterm"); log("Stopped (SIGTERM)", "INFO"); exec("termux-wake-unlock", () => {}); process.exit(0); });
process.on("uncaughtException",  e => { clearBotStatus("uncaught_exception"); log(`💥 Uncaught: ${e.message}`, "ERROR"); notify("⚠️ Bot Error", e.message.slice(0, 100)); process.exit(1); });
